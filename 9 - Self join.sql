...
FROM
  ...
  JOIN stops stopb ON (b.stop = stopb.id)
WHERE
  stopa.name = 'Craiglockhart' -- Find the routes involving two buses that can go from Craiglockhart to Lochend.
...
FROM
  ...
  JOIN (
    ...
    WHERE
      stopz.name = 'Lochend'
  ) AS finish ON (start.name = finish.name);